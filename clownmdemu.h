...
class ClownMDEmuCXX : protected ClownMDEmu
{
	////////////////
	// Subclasses //
	////////////////
...
#define CLOWNMDEMU_CONFIGURATION_GETTER_SETTER_NOT(IDENTIFIER, VALUE) \
	...
	{
	public:
		StateBackup(const ClownMDEmuCXX &emulator)
		{
			ClownMDEmu_SaveState(&emulator, this);
		}

		void Apply(ClownMDEmuCXX &emulator) const
		{
...
private:
	...
	{
		static_cast<ClownMDEmuCXX*>(user_data)->ColourUpdated(index, colour);
	}
	...
	{
		static_cast<ClownMDEmuCXX*>(user_data)->ScanlineRendered(scanline, pixels, left_boundary, right_boundary, screen_width, screen_height);
	}
	static cc_bool Callback_InputRequested(void* const user_data, const cc_u8f player_id, const ClownMDEmu_Button button_id)
	{
		return static_cast<ClownMDEmuCXX*>(user_data)->InputRequested(player_id, button_id);
	}
	...
	{
		static_cast<ClownMDEmuCXX*>(user_data)->FMAudioToBeGenerated(clownmdemu, total_frames, generate_fm_audio);
	}
	...
	{
		static_cast<ClownMDEmuCXX*>(user_data)->PSGAudioToBeGenerated(clownmdemu, total_frames, generate_psg_audio);
	}
	...
	{
		static_cast<ClownMDEmuCXX*>(user_data)->PCMAudioToBeGenerated(clownmdemu, total_frames, generate_pcm_audio);
	}
...
public:
	ClownMDEmuCXX(const InitialConfiguration &configuration)
	{
